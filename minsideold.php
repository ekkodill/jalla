...
if(isset($_POST['sendmail'])) {
  if(!empty($_POST['frommail']) && !empty($_POST['eposttil']) && !empty($_POST['mailarea']) && !empty($_POST['mailtittel'])) {
            ...
            if(mailUsers($tittel, $melding, $fra, $til)) {
              header("location: minsideold.php?mailsendt");
            } else {
          header("location: minsideold.php?mailerror");
         }
  }
} 
  ...
  $epost = sanitize(trim($_POST['upepost']));
      ...
      if(!empty($fnavn) && !empty($enavn) && !empty($epost)) {      
          if(endreBrukerInfo($brukerPK, $fnavn, $enavn, $epost)) {
                header('Location: minsideold.php?oppdatert');
          } else { header('Location: minsideold.php?error'); }
      } else { header('Location: minsideold.php?tomerror'); }
    } else { header('Location: minsideold.php?eposterror'); }
  }
}
        ...
        <div id="page">
          <section>
            <div class="msvenstre"><div class="bpbilde"><img src="img/mann.jpg" height:"30%" width="70%"alt="bilde"></div>
              <div class="profinfo">
                <form name="profil" action="minsideold.php" method="POST">
...
<?php  if($user_data['brukertype'] != 3) { ?>
        <form class="fasplas" action="" method="POST">
           ...
           <label>Velg mottakere:</label>
             <select name="eposttil" class="dropned" action="minsideold.php" method="POST">
          ...
          if($user_data['brukertype'] == 3 && count(sjekkAntall("oppgaver 
           ...
           <form class="fasplas"action="minsideold.php" method="post">
    ...
    <?php } ?>
            ...
            <div id="minside">
                <form id="byttpw" name="reg" method="post" action="minsideold.php">
                 <h2>Bytt passord</h2>
                    Gammelt passord: 
